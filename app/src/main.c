...
// fprintf(stderr, "1\n");
	...
	hantek_write(han_fd, "dds:freq 500");
	hantek_write(han_fd, "dds:amp 0");
...
// fprintf(stderr, "4\n");
		memset(buf, 0, 100);
		snprintf(buf, 100, "dds:amp %.3lf", 2*han_amp);
		r = hantek_write(han_fd, buf);
		if(r < 0)
		{
			fprintf(stderr, "# E: Unable to set amp (%d)\n", r);
			set_run(0);
			continue;
		}
